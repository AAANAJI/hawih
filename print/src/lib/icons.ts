/**
 * icons.ts — the store's ONLY icon source. Inline stroke SVGs (lucide-style
 * geometry, 24 viewBox, stroke=currentColor, width 1.75, round caps).
 * No emoji anywhere in the store — every pictogram renders from this map,
 * either via <Icon> (Astro) or iconSvg() (JS-rendered DOM, e.g. catalog-sync),
 * so the two can never drift.
 *
 * Directional icons (chevron-start, arrow-end) must carry the `pk-icon--flip`
 * class so they mirror under [dir='rtl'] (see base.css).
 */
...
export const ICON_PATHS: Record<IconName, string> = {
  ...
  'chevron-start': '<path d="m9 18 6-6-6-6"/>',
  'arrow-end': '<path d="M5 12h14"/><path d="m13 6 6 6-6 6"/>',
...
export function iconSvg(name: IconName, cls = ''): string {
  const classes = cls ? `pk-icon ${cls}` : 'pk-icon';
  return (
